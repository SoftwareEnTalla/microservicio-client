...
@Injectable()
export class ClientSegmentQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(ClientSegmentQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: ClientSegmentQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${ClientSegment.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const clientsegments = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de clientsegments obtenido con éxito",
        data: clientsegments,
  ...
  async findById(id: string): Promise<ClientSegmentResponse<ClientSegment>> {
    try {
      const clientsegment = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "ClientSegments no encontrados para la propiedad y valor especificado"
        );
      // Devolver clientsegment
      return {
        ok: true,
        message: "ClientSegments obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<ClientSegmentResponse<ClientSegment>> {
    try {
