 ...
 * Client nuevo (flujo anti-duplicación).
 *
 * Algoritmo: pg_trgm+tsvector (delegado al upstream service).
 * Campos indexados: firstName,lastName,documentType,documentNumber,phone
 */
import { Controller, Get, Query, HttpStatus, Res } from '@nestjs/common';
import { ApiOperation, ApiTags, ApiQuery } from '@nestjs/swagger';
import type { Response } from 'express';
import { ClientUpstreamClientService } from '../services/client-upstream-client.service';

const SIMILARITY_FIELDS: string[] = ['firstName', 'lastName', 'documentType', 'documentNumber', 'phone'];
const DEFAULT_MAX = 20;

@ApiTags('client')
@Controller('clients')
export class ClientUpstreamSearchController {
  constructor(private readonly upstreamClient: ClientUpstreamClientService) {}
