  ...
  export class ClientQueryRepository {
    ...
    client: LoggerClient.getInstance()
      ...
      .get(ClientRepository.name),
        ...
        async findOne(options?: Record<string, any>): Promise<Client | null> {
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) Client:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<Client> = options as FindOptionsWhere<Client>;
