...
export class ClientCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: Client): Promise<Client> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvt1 = new ClientCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: Client[]): Promise<Client[]> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvts2 = result.map((el)=>new ClientCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      const __dualEvt3 = new ClientUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            const __dualEvt4 = new ClientUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       const __dualEvt5 = new ClientDeletedEvent(id, {
        instance: entity,
        metadata: {
