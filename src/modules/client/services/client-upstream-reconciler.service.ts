 ...
 *      como comando 'register-person-from-client' al upstream.
...
import { ClientUpstreamClientService } from './client-upstream-client.service';

@Injectable()
export class ClientUpstreamReconcilerService
  implements OnModuleInit, OnModuleDestroy
{
  private readonly logger = new Logger(
    ClientUpstreamReconcilerService.name,
  );
  ...
  constructor(
    ...
    private readonly upstreamClient: ClientUpstreamClientService,
  ) {}
  ...
  async run(): Promise<void> {
    ...
    for (const record of pending) {
      try {
        ...
        // NOTA: la publicación Kafka del comando register-person-from-client
        // se gestiona desde el command handler estándar; aquí sólo marcamos PENDING.
      } catch (err) {
        (record as any).upstreamLastErrorAt = new Date();
        await this.repository.save(record);
        this.logger.warn(
          `Fallo empujando client ${(record as any).id} al upstream`,
        );
      }
    }
