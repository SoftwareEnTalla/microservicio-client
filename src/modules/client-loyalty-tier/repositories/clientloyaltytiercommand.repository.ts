...
export class ClientLoyaltyTierCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: ClientLoyaltyTier): Promise<ClientLoyaltyTier> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publish(new ClientLoyaltyTierCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: ClientLoyaltyTier[]): Promise<ClientLoyaltyTier[]> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publishAll(result.map((el)=>new ClientLoyaltyTierCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      this.eventPublisher.publish(new ClientLoyaltyTierUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            this.eventPublisher.publish(new ClientLoyaltyTierUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       this.eventPublisher.publish(new ClientLoyaltyTierDeletedEvent(id, {
        instance: entity,
        metadata: {
