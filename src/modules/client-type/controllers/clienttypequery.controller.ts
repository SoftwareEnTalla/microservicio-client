...
import { ClientTypeDto } from "../dtos/all-dto";

import { logger } from '@core/logs/logger';

@ApiTags("ClientType Query")
@Controller("clienttypes/query")
export class ClientTypeQueryController {
  #logger = new Logger(ClientTypeQueryController.name);

  constructor(private readonly service: ClientTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all clienttype with optional pagination" })
  @ApiResponse({ status: 200, type: ClientTypesResponse })
  @ApiQuery({ name: "options", required: false, type: ClientTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ClientType>    
  ): Promise<ClientTypesResponse<ClientType>> {
    try {
     
      const clienttypes = await this.service.findAll(options);
      logger.info("Retrieving all clienttype");
      return clienttypes;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get clienttype by ID" })
  @ApiResponse({ status: 200, type: ClientTypeResponse<ClientType> })
  @ApiResponse({ status: 404, description: "ClientType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the clienttype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ClientTypeResponse<ClientType>> {
    try {
      const clienttype = await this.service.findOne({ where: { id } });
      if (!clienttype) {
        throw new NotFoundException(
          "ClientType no encontrado para el id solicitado"
        );
      }
      return clienttype;
    } catch (error) {
  ...
  @ApiOperation({ summary: "Find clienttype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter clienttype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ClientTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ClientType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find clienttypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ClientType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all clienttypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count clienttypes with conditions" })
  @ApiResponse({ status: 200, type: ClientTypesResponse<ClientType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ClientTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one clienttype with conditions" })
  @ApiResponse({ status: 200, type: ClientTypeResponse<ClientType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiOperation({ summary: "Find one clienttype or return error" })
  @ApiResponse({ status: 200, type: ClientTypeResponse<ClientType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ClientType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
