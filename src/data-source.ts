...
export const AppDataSource = new DataSource({
  ...
  database: process.env.DB_NAME || "client-service",
...
function interpolateSqlTemplate(sql: string): string {
  return sql
    .replace(/\$\{SQL_SHA256:([A-Z0-9_]+)\}/g, (_m, name) => escapeSqlLiteral(sha256(process.env[name] || "")))
    .replace(/\$\{SHA256:([A-Z0-9_]+)\}/g, (_m, name) => sha256(process.env[name] || ""))
    .replace(/\$\{SQL:([A-Z0-9_]+)\}/g, (_m, name) => escapeSqlLiteral(process.env[name] || ""))
...
async function resolveDatabaseScripts(scriptDirectory: string, dbType: string): Promise<string[]> {
  ...
  try {
    ...
    // sin init-order.txt → orden alfabético
  }
  return sqlFiles;
}
...
async function runDatabaseInitializationScripts() {
  ...
  if (dbType !== "postgres") {
    logger.warn("⚠️ DB_TYPE='" + dbType + "' no tiene ejecutor SQL implementado. Se omiten scripts de src/database.");
    return;
  }
